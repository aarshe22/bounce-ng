...
try {
    ...
    // SQLite GROUP_CONCAT doesn't support DISTINCT, so we'll handle uniqueness in PHP
    $stmt = $db->query("
        SELECT 
            original_to,
    ...
    // Format the data - remove duplicates from GROUP_CONCAT results
    foreach ($badAddresses as &$address) {
        $smtpCodesList = !empty($address['smtp_codes']) 
            ? explode(',', $address['smtp_codes']) 
            : [];
        $address['smtp_codes'] = array_values(array_unique(array_filter(array_map('trim', $smtpCodesList))));
        
        $domainsList = !empty($address['domains']) 
            ? explode(',', $address['domains']) 
            : [];
        $address['domains'] = array_values(array_unique(array_filter(array_map('trim', $domainsList))));
    }
